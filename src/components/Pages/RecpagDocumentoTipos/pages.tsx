...
export function RecpagDocumentoTipoPage() {
  ...
  const deleteRecpagDocumentoTipo = useDeleteRecpagDocumentoTipo();
  const {
    data,
    isLoading,
    isError,
  ...
  const deleteItem = async (password: string) => {
    try {
      if (!id) return;
      const res = await deleteRecpagDocumentoTipo.mutateAsync({ id, password });
      if (res && res.error === "") {
        setShowDialog(false);
        toast.success(
          res.message ||
            "Exclusão de tipo de rec/pag realizada com êxito"
        );
      }
    } catch (e) {
      const { message } = handleApiError(e);
      toast.error(message || "Erro ao excluír tipo de rec/pag");
    }
  };

  const addItem = () => {
    if (!hasPermission("recpag_documento_tipos_create")) {
      toast.warning(
        messageToastHelper.accessDenied(
          "o cadastro de tipo de rec/pag"
        )
      );
      return;
    }
  ...
  const actions: TableActions<IRecpagDocumentoTipo> = {
    onUpdate: (id) => {
      if (!hasPermission("recpag_documento_tipos_update")) {
        toast.warning(
          messageToastHelper.accessDenied(
            "a alteração de tipo de rec/pag"
          )
        );
        return;
      }
    ...
    onDelete: (id) => {
      if (!hasPermission("recpag_documento_tipos_delete")) {
        toast.warning(
          messageToastHelper.accessDenied(
            "a exclusão de tipo de rec/pag"
          )
        );
        return;
      }
    ...
    onDetails: (id) => {
      if (!hasPermission("recpag_documento_tipos_findOne")) {
        toast.warning(
          messageToastHelper.accessDenied(
            "o detalhes de tipo de rec/pag"
          )
        );
        return;
      }
  ...
  return (
    <>
      <div className="space-y-2 bg-background p-4 h-full flex flex-col">
        <CustomHeader title="Tipos de Rec/Pag" />
      ...
      <ResponsiveModal
        title={
          isCreate
            ? "Novo tipo de rec/pag"
            : "Atualizar tipo de rec/pag"
        }
        description={
          isCreate
            ? "Preencha os dados do tipo de rec/pag e clique em salvar."
            : "Atualize os dados do tipo de rec/pag e clique em salvar."
        }
